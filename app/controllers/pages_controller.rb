class PagesController < ApplicationController
  ...
  def top
    ...
    #binding.pry
    @rankings = Post.joins(:bravo_users).where("bravos.created_at like ?", "%#{Date.current}%").distinct.sort {|a, b| (b.bravo_users.count <=> a.bravo_users.count).nonzero? || (b.impressions_count <=> a.impressions_count)}.take(10)
